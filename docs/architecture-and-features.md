...
| **olo-internal-features** | Aggregates kernel-privileged features: **InternalFeatures.registerInternalFeatures(registry, sessionCache, runLedgerOrNull, executionEventSinkOrNull)** registers DebuggerFeature, QuotaFeature, MetricsFeature; (if ledger enabled) RunLevelLedgerFeature, NodeLedgerFeature; (if execution event sink provided) **ExecutionEventsFeature**. **InternalFeatures.clearLedgerForRun()** for run cleanup. Worker depends on this module and calls it at startup; no direct dependency on olo-feature-debug, olo-feature-quota, olo-feature-metrics from olo-worker. |
...
| **olo-run-ledger** | **Run ledger** (env-gated by **OLO_RUN_LEDGER**). **Write-only, fail-safe, append-only.** Two paths: **runExecutionTree** (one runId per activity, snapshot passed to NodeExecutor); **executeNode** (per-node) reuses **runId from plan** (getExecutionPlan puts runId in plan JSON; every executeNode reuses it so one olo_run per logical run); **runStarted** only on first node in plan; **LedgerContext** (thread-local) set in activity and in NodeExecutor (ledgerRunId). **LedgerStore**, **NoOpLedgerStore**, **JdbcLedgerStore** (PostgreSQL: **olo_run**, **olo_run_node**, **olo_config**; **UUID**; **PGobject** for JSONB to avoid driver placeholder double-count); **RunLedger**; **NodeAiMetrics**, **NodeReplayMeta**, **NodeFailureMeta**. **RunLevelLedgerFeature** (root); **NodeLedgerFeature** (every node). **Execution events for chat UI**: **ExecutionEventSink**, **InMemoryExecutionEventSink**, **ExecutionEventsFeature** (auto-attached like ledger-node); runId from workflow input **context.runId** when present. See docs/run-ledger-schema.md and docs/execution-events.md. |
...
| **olo-worker** | `OloWorkerApplication`: calls **OloBootstrap.initializeWorker()**; uses **WorkerBootstrapContext** (config, queues, tenants, runLedger, sessionCache, **pluginExecutorFactory**, **runResourceCleanup**). **OloKernelActivitiesImpl**(sessionCache, allowedTenantIds, runLedger, **pluginExecutorFactory**); **ExecuteNodeDynamicActivity** (per-node activity when linear; type "NODETYPE" or "PLUGIN:pluginRef"). **getExecutionPlan** adds **runId** to plan JSON for per-node path (single run per logical run). **executeNode** reuses plan runId, sets LedgerContext, calls runStarted only on first node, runEnded in finally; passes runId to NodeExecutor. **runExecutionTree** creates one runId, sets LedgerContext, snapshot has runId for NodeExecutor. Workflow: getExecutionPlan; if linear → executeNode per leaf; else runExecutionTree. INCR activeWorkflows; LocalContext, snapshot, **ExecutionEngine.run(...)**; DECR in finally. NodeExecutor, PluginInvoker, ResultMapper. Shutdown: **ctx.runResourceCleanup()**. |
...
- **Run ledger** (optional): When **OLO_RUN_LEDGER=true**, RunLevelLedgerFeature (root) and NodeLedgerFeature (every node) are registered. **Ledger is write-only and append-only.** **Two execution paths:** (1) **runExecutionTree** — one activity runs the full tree; one runId per run; LedgerContext and runStarted/runEnded in the activity; NodeExecutor receives runId from ExecutionConfigSnapshot. (2) **executeNode (per-node/linear)** — one activity per node; **getExecutionPlan** adds **runId** to the plan JSON; every **executeNode** reuses that runId so **one olo_run row per logical run**; **runStarted** is called only for the **first node** in the plan (**isFirstNodeInPlan**); **runEnded** in finally for every node (last update wins). **LedgerContext** (thread-local) is set in the activity and in NodeExecutor so NodeLedgerFeature always sees a non-null runId. Each node attempt is recorded in **olo_run_node**; uniqueness **(run_id, node_id, attempt)**. Config snapshot in **olo_config** (immutable per run). For **MODEL**/ **PLANNER**, NodeLedgerFeature persists token/cost, replay and failure meta. **JdbcLedgerStore** uses **PGobject** for JSONB columns to avoid PostgreSQL JDBC placeholder double-count; **attempt** column at node start. See docs/run-ledger-schema.md (including **Run ledger implementation details**).
